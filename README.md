...
**v1.5.0.0**<br/>
- Исправлена критическая проблема: приложение молча исчезало из памяти через 30–60 минут на Windows 11.
- Добавлена полная защита хуков клавиатуры и мыши от исключений (Windows 11 молча удаляет хуки при ошибках).
- Добавлен автоматический мониторинг и переустановка хуков, если Windows их удалит.
- Добавлены глобальные обработчики необработанных исключений для всех потоков.
- Исправлен бесконечный цикл в буфере обмена, блокировавший хук.
- Исправлен ошибочный выход из программы при нажатии на всплывающую подсказку трея.
- Добавлена совместимость с Windows 10/11 в манифесте приложения.
- Улучшено логирование для диагностики проблем.

**v2.0.0.0**<br/>
- Проект мигрирован с .NET Framework 4.5 на .NET 10 (современный SDK-формат).
- NLog обновлён до версии 5.x.
- Удалена зависимость Costura.Fody (заменена встроенной публикацией single-file).
- Исправлена совместимость API: Process.Start для URL, Assembly.Location для single-file.
...
**v1.5.0.0**<br/>
- Fixed critical issue: application was silently disappearing from memory after 30-60 minutes on Windows 11.
- Added full exception protection for keyboard and mouse hooks (Windows 11 silently removes hooks on errors).
- Added automatic hook health monitoring and re-installation if Windows removes them.
- Added global unhandled exception handlers for all threads.
- Fixed infinite loop in clipboard operations that was blocking the hook callback.
- Fixed accidental app exit when clicking on tray balloon notification.
- Added Windows 10/11 compatibility in application manifest.
- Improved logging for diagnostics.

**v2.0.0.0**<br/>
- Migrated project from .NET Framework 4.5 to .NET 10 (modern SDK-style project).
- NLog upgraded to version 5.x.
- Removed Costura.Fody dependency (replaced with built-in single-file publish).
- Fixed API compatibility: Process.Start for URLs, Assembly.Location for single-file.
